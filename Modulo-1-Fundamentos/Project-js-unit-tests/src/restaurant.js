...
const createMenu = (obj) => {
  const myMenu = {
    fetchMenu: () => obj,
    consumption: [],
    order: (order) => myMenu.consumption.push(order),
    pay: () => {
      const menu = myMenu.fetchMenu();
      let total = 0;

      for (let index = 0; index < myMenu.consumption.length; index += 1) {
        const itemAtual = myMenu.consumption[index];

        if (menu.food[itemAtual]) {
          total += menu.food[itemAtual];
        } else if (menu.drink[itemAtual]) {
          total += menu.drink[itemAtual];
        }
      }
